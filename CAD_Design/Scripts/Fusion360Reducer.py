...
def run(context):
    ...
    try:
        ...
        # Collect mesh bodies from the root and every occurrence.
        mesh_bodies = []
        for body in root.meshBodies:
            mesh_bodies.append(body)
        for occ in root.allOccurrences:
            for body in occ.component.meshBodies:
                mesh_bodies.append(body)
 
        if not mesh_bodies:
            ui.messageBox(
                'No mesh bodies found in this design.\n\n'
        ...
        report_lines = []
        for i, body in enumerate(mesh_bodies):
            tri_count = _safe_triangle_count(body)
            report_lines.append(f'  [{i}] "{body.name}"  -  ~{tri_count:,} triangles')
 
        if len(mesh_bodies) == 1:
            target = mesh_bodies[0]
        else:
            choice = ui.inputBox(
                'Multiple mesh bodies found:\n' + '\n'.join(report_lines) +
        ...
        ui.messageBox(
            f'Selected mesh body: "{target.name}"\n'
